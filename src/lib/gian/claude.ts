...
export async function generateDraft(args: {
  ...
  const facts = [
    `오늘 날짜: ${today} — 문서의 연도는 이 값을 쓴다. 월·일이 입력에 있으면 그대로 쓴다.`,
    `단지명: ${tenantName}`,
    ...
    // 빈칸(○월 ○일)은 채워지지 않은 채 서명까지 받으면 결재본에 영구히 남는다 —
    // 미정이면 완결된 문장으로 적는 게 결재 문서의 사실 기록이다(사용자 확정 2026-08-01)
    `일정 표기 규칙: "○월 ○일"처럼 빈칸(○)이 든 날짜는 어떤 경우에도 쓰지 않는다. 입력된 일정 표현("9월 중" 등 대략 표기 포함)은 다듬거나 날짜꼴로 바꾸지 말고 한 글자도 다르지 않게 그대로 쓰되("9월 초"를 "9월 중"으로 바꾸지 않는다), 시행·공사 일정 항목에만 쓴다. "계약체결" 항목은 계약이 결재 뒤에 이뤄지므로 — 입력에 계약 날짜가 구체적으로 명시된 경우가 아니면 — 월도 적지 말고 "결재 후 계약 체결 시 확정"으로만 적는다. 날짜를 모르는 그 밖의 항목은 "결재 후 계약 체결 시 확정"(계약 없는 기안이면 "추후 확정")으로 적는다.`,
    form.schedule
      ? ""
      : `일정이 미입력이다 — 추진일정 절을 "계약체결"·"공사시행" 같은 세부 항목(가. 나.)으로 나누지 말고, 항목 기호 없이 "결재 후 계약 체결 시 확정" 한 줄만 적는다.`,
    cls.amountRaw > 0
