 ...
 *
 * 여기는 로그인 전용이다. 카카오로 새 단지를 만드는 경로는 아직 없고(가입은
 * `/signup`), 이건 미구현이지 제품 모델이 아니다 — 디벅은 가입·체험·구독·결제를
 * 사용자가 스스로 끝내는 셀프서비스다. 카카오 신규 가입도 언젠가 붙는다.
 *
 * 매칭 순서: kakaoId로 연결된 계정 → 없으면 인증된 이메일이 같은 기존 계정에
 * 최초 1회 연결 → 그것도 없으면 가입을 안내하며 로그인 화면으로 돌려보낸다.
 */
